...
local function readtable()
	...
	plyname = string.lower(ply:Name())
	plyname, rpcount = string.gsub(plyname, "[\128-\244]+", "a")
		if(file.Exists("showid/"..plyname..".txt","DATA") == true) then
			Player_Table_Import = util.JSONToTable(file.Read("showid/"..plyname..".txt","DATA"))
			if(Player_Table_Import.Language != nil) then
					ShowID.Language = tonumber(Player_Table_Import.Language)
					DefineLanguageTable()
			else 
				ShowID.Language = 1
				DefineLanguageTable()
...
sound.PlayFile( "sound/showid/open_book.mp3", "noplay", function( station, errCode, errStr )
	...
	DLabel:SetText( ShowID.LanguageSet.Text01 )
	...
	DLabel:SetText( ShowID.LanguageSet.Text02 )
	DLabel:SetFont("ShowID24")
	...
	DLabel:SetText( ShowID.LanguageSet.Text03 )
	DLabel:SetFont("ShowID24")
	...
	DLabel:SetText( ShowID.LanguageSet.Text04 )
	DLabel:SetFont("ShowID24")
	...
	DLabel:SetText( ShowID.LanguageSet.Text05 )
	DLabel:SetFont("ShowID24")
	...
	DLabel:SetText( ShowID.LanguageSet.Text06 )
	DLabel:SetFont("ShowID24")
	...
	DLabel:SetText( ShowID.LanguageSet.Text07 )
	DLabel:SetFont("ShowID24")

	local DermaButton = vgui.Create( "DButton", DFrame ) // Create the button and parent it to the frame
	DermaButton:SetText( ShowID.LanguageSet.Text08 )					// Set the text on the button
...
local function createdir()
	...
	if(file.Exists("showid/"..plyname..".txt","DATA") == false) then
	...
	local tabletowrite = util.TableToJSON(Player_Table)
		file.Write("showid/"..plyname..".txt",tabletowrite)
	end
end
...
local function showidconfig()
	...
	DFrame:SetSize(Scrw * 0.16, Scrh * 0.46)               -- Set the size to 300x by 200y
	...
	TextEntry1:SetPlaceholderText( ShowID.LanguageSet.Text09 )
	...
	TextEntry5:SetPlaceholderText( ShowID.LanguageSet.Text10 )
	...
	TextEntry6:SetPlaceholderText( ShowID.LanguageSet.Text11 )
	...
	TextEntry2:SetPlaceholderText( ShowID.LanguageSet.Text12 )
	...
	TextEntry3:SetPlaceholderText( ShowID.LanguageSet.Text06 )
	...
	TextEntry4:SetPlaceholderText( ShowID.LanguageSet.Text13 )

	local DComboBox = vgui.Create( "DComboBox", DFrame )
	DComboBox:SetPos( Scrw * 0.013, Scrh * 0.36 )
	DComboBox:SetSize( Scrw * 0.13, Scrh * 0.02 )
	DComboBox:SetValue( "English" )
	DComboBox:AddChoice( "English" )
	DComboBox:AddChoice( "Swedish" )
	DComboBox.OnSelect = function( self, index, value )
	print(DComboBox:GetSelectedID())
	end


	local DermaButton = vgui.Create( "DButton", DFrame )
	DermaButton:SetText( ShowID.LanguageSet.Text14 )
	DermaButton:SetPos( Scrw * 0.013, Scrh * 0.4 )
		DermaButton:SetFont("ShowID20")
	DermaButton:SetSize( Scrw * 0.13, Scrh * 0.05 )
	DermaButton.DoClick = function()
		if(TextEntry1:GetValue() == "" || TextEntry2:GetValue() == "" || TextEntry3:GetValue() == "" || TextEntry4:GetValue() == "" || TextEntry5:GetValue() == "" || TextEntry6:GetValue() == "") then ply:PrintMessage( HUD_PRINTTALK, ShowID.LanguageSet.Text15 ) return end
